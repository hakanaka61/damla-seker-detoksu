        ...
        {activeTab === 'form' && (
          <div className="bg-white/70 backdrop-blur-xl rounded-[2rem] p-6 shadow-xl border border-white animate-in fade-in zoom-in duration-300">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {/* YENİ NET BUTONLAR: ŞEKER YEDİM Mİ, YEMEDİM Mİ? */}
              <div className="flex space-x-3">
                <button 
                  type="button" 
                  onClick={() => setFormData({...formData, seker_tuketimi: true})}
                  className={`flex-1 p-4 rounded-2xl font-bold text-sm transition-all flex flex-col items-center justify-center space-y-1 ${formData.seker_tuketimi ? 'bg-green-500 text-white shadow-lg scale-105' : 'bg-green-50 text-green-600 border border-green-200'}`}
                >
                  <span className="text-2xl">🎉</span>
                  <span>Şekersiz Geçti</span>
                </button>
                <button 
                  type="button" 
                  onClick={() => setFormData({...formData, seker_tuketimi: false})}
                  className={`flex-1 p-4 rounded-2xl font-bold text-sm transition-all flex flex-col items-center justify-center space-y-1 ${!formData.seker_tuketimi ? 'bg-red-500 text-white shadow-lg scale-105' : 'bg-red-50 text-red-600 border border-red-200'}`}
                >
                  <span className="text-2xl">😔</span>
                  <span>Şeker Yedim</span>
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="flex flex-col items-center justify-center p-3 rounded-2xl border border-gray-200 bg-gray-50 cursor-pointer">
              ...
              <button type="submit" className="w-full bg-pink-500 hover:bg-pink-600 text-white font-extrabold rounded-2xl p-4 shadow-lg active:scale-95 transition-all">Günü Kaydet 🚀</button>
